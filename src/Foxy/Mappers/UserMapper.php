...
SET email = :email, username = :username, password = :password, first_name = :firstName, last_name = :lastName, last_login = :lastLogin, avatar_img = :avatarImg, active = :active, token = :token
WHERE id = :id
LIMIT 1;
EOT;
      $statement = $this->pdo->prepare($query);
    ...
    {
      $user = new User($row->email, $row->password, $row->username, $row->first_name, $row->last_name, $row->avatar_img, false);
